...
async def publish(url: str, email: str, password: str, audio_file: str, title: str, description: str,
                  ...
                  thumbnail: Optional[str] = None, is_publish: bool = True):
    async with async_playwright() as p:
        ...
        additional_detail = page.get_by_role("button", name="Additional details (optional)")
        ...
        if thumbnail:
            if os.path.exists(thumbnail):
                ...
                async with page.expect_file_chooser() as fc_info:
                    await page.get_by_role("button", name="Episode cover art Change").click()
                file_chooser = await fc_info.value
                ...
                print("Skip thumbnail replacement because the file does not exist.:{thumbnail}")
